...
import {
  ...
  CODE_EXPIRATION_MS,
} from "@/lib/verification-shared"; // Import from the new shared file
...
export async function GET(request: NextRequest) {
  ...
  console.log(
    `[/api/get-verification-code] Received request for phoneNumber: ${phoneNumberParam}`
  );
  ...
  if (chatIdParam && attempt.telegramChatId !== Number(chatIdParam)) {
    return NextResponse.json(
      {
        success: false,
        message:
          "Unauthorized: This code was not requested by this Telegram user.",
      },
      { status: 403 }
    );
  }
  ...
  console.log(
    `[/api/get-verification-code] Successfully retrieved code ${attempt.code} for ${validatedPhoneNumber}.`
  );
  return NextResponse.json(
    {
      success: true,
